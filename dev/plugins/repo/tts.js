	...
	var plugin = function(){
					...
					if($lib.browser != "msie"){
						...
						{//For HTML5
							...
							$lib.getJSON(settings.speechServicesURL + "cache/request.php?id=" + arg.id + "&callback=?").done(function(jsonDocument){
								index = 0;
								if($.isArray(jsonDocument.trackList.track))//Store the audio urls to be played later by the audio html5 element
									$.each(jsonDocument.trackList.track, function(key, value){
		...
		AtKit.addButton(
			...
			function(dialogs, functions){
				...
				$lib('#sbStartTTS').on('click touchend', function(e){ 
				...
				$lib('#sbStartTTSSelection').on('click touchend', function(e){ 
					...
					//selectedData = AtKit.call('getSelectedText');
		...
		if(navigator.userAgent.match(/(iPhone|iPod|iPad)/i))
		{
			setInterval(function(e){
				selectedDataIOS = AtKit.call('getSelectedText');
			}, 100);
		}
