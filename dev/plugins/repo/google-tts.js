(function(){
	...
	var plugin = function(){
		...
		var settings = {
			...
			"ttsChunkSize": 90 // 97 is max character length for Google translate TTS service
		};
		...
		var TTSDialogs = {
			"options": {
				...
				"body": AtKit.localisation("tts_what") + "<br /><button id=\"sbStartGoogleTTSSelection\">" + AtKit.localisation("tts_selected") + "</button>"
			},
			"starting": {
		...
		AtKit.addFn('getSelectedTextGoogleTTS', function(strip){
			
			var text = AtKit.call('getSelectedTextInElementGoogle');
		...
		AtKit.addFn('getSelectedTextInElementGoogle', function(){
		...
		AtKit.addFn('sendGoogleTTSChunk', function(args){
			if(args.block == 1){
				var start = 0;
			} else {
			...
			$lib.getJSON(urlString, function(RO){
				...
				if(args.block == args.totalBlocks){
					...
					if(RO.status == "encoding"){
						AtKit.call('countdownGoogleTTS', { 'timeLeft':(RO.est_completion / RO.chunks), 'id': RO.ID });
					...
					if(RO.data.message == "ChunkSaved"){
						AtKit.call('sendGoogleTTSChunk', { 'fullData':args.fullData, 'block':(args.block + 1), 'totalBlocks':args.totalBlocks, 'reqID':args.reqID });
					} else {
		...
		AtKit.addFn('countdownGoogleTTS', function(arg){
			if(isNaN(arg.timeLeft)){
				...
				if(arg.timeLeft == 0){
					...
					if($lib.browser != "msie"){
						...
						AtKit.call('setupGoogleTTSListeners');
					} else {
					...
					window.setTimeout(function(){ AtKit.call('countdownGoogleTTS', { 'timeLeft':(arg.timeLeft - 1), 'id':arg.id }) }, 1000);
				}
			}
		});
		
		AtKit.addFn('setupGoogleTTSListeners', function(args){
		...
		AtKit.addFn('sbStartGoogleTTSSelection', function(){
						
			AtKit.set('TTS_clickEnabled', false);

			var selectedData = AtKit.get('TTSselectedData');
			if(selectedData == "" || typeof selectedData == "undefined") selectedData = AtKit.call('getSelectedTextGoogleTTS');
			...
			this.clickEnabled = false;
				...
				if(chunks > 0){
					...
					AtKit.call('sendGoogleTTSChunk', { 'fullData':transmitData, 'block':1, 'totalBlocks':chunks, 'reqID':reqID, });
				} else {
					AtKit.message( "<h2>" + AtKit.localisation("tts_error") + "</h2><p>" + AtKit.localisation("tts_problem") + "</p>" );
				}
		...
		$lib(document).delegate('#at-btn-tts', 'mousemove, focus, mouseover', function(){
			var text = AtKit.call('getSelectedTextGoogleTTS');
		...
		AtKit.addButton(
			...
			function(dialogs, functions){
				...
				AtKit.call('sbStartGoogleTTSSelection', {});
				
				/*
				$lib('#sbStartGoogleTTSSelection').click(function(){
					AtKit.call('sbStartGoogleTTSSelection', {});
				});
				*/
